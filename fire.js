...
const debug = false;
const windLeft = false;
const windRight = false;
const intervalFirePropagation = 50
...
function start(){
    ...
    setInterval(calculateFirePropagation, intervalFirePropagation);
}
...
function updateFireIntensityPerPixel(index){
    const belowPixelIndex = index + fireWidth;
    ...
    if(windLeft){
        firePixelArray[index - decay] = newFireIntensity;
    }else if(windRight){
        firePixelArray[index + decay] = newFireIntensity;
    }else{
        firePixelArray[index] = newFireIntensity;
    }
}
